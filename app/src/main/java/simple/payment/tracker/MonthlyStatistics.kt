...
data class MonthlyReport(val month: String, val payments: List<Payment>) {
  fun print() {
    println("Month: $month")
...
class MonthlyStatistics(private val payments: Observable<List<Payment>>) {
  ...
  companion object {
    fun monthly(merged: List<Payment>): List<MonthlyReport> {
      ...
      return monthly.entries.reversed().map { (month, payments) -> MonthlyReport(month, payments) }
    }
  }
}
