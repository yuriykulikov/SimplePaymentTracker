...
class LogbackLoggerFactory(private val slf4j: org.slf4j.ILoggerFactory) : LoggerFactory {
  override fun createLogger(tag: String): Logger {
    val logbackLogger = slf4j.getLogger(tag)
    val levelEnabled = { level: Level ->
      when (level) {
        Level.TRACE -> logbackLogger.isTraceEnabled
        Level.DEBUG -> logbackLogger.isDebugEnabled
        Level.INFO -> logbackLogger.isInfoEnabled
        Level.WARNING -> logbackLogger.isWarnEnabled
        Level.ERROR -> logbackLogger.isErrorEnabled
      }
    }
    val log = { level: Level, e: Throwable?, message: String ->
      when (level) {
        Level.TRACE -> logbackLogger.trace(message, e)
        Level.DEBUG -> logbackLogger.debug(message, e)
        Level.INFO -> logbackLogger.info(message, e)
        Level.WARNING -> logbackLogger.warn(message, e)
        Level.ERROR -> logbackLogger.error(message, e)
      }
    }
    return Logger(levelEnabled, log)
  }
}
